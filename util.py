...
#             print(d)
        ...
        nP = np.array([f'{x:g}' for x in nP])
        nP_positions, = (np.array(in_tokens) == 'NUM').nonzero()
        d.update(dict(
            in_tokens=in_tokens, out_tokens=out_tokens,
            nP=nP, nP_positions=nP_positions
        ))
    constants = ['%g' % n for n, count in constant_counts.items() if count >= n_min_vocab]
    return constants, n_max_nP

def infix_to_prefix(expression):
    op_priority = {'+': 0, '-': 0, '*': 1, '/': 1, 'm': 3, 'l': 3, '^': 2}
...
def setup(use_t5, path='data/train.json', n_min_vocab=5, seed=0, test_split=0.2, data=None):
    ...
    return train_data, test_data, in_vocab, out_vocab, n_max_nP, t5_model
